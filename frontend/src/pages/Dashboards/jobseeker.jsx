...
// import React, { useState, useEffect } from "react";
// import { Card, Button, message, Tabs, Layout, Typography, Avatar, Empty } from "antd";
// import { LogoutOutlined, UserOutlined, ApartmentOutlined, FileDoneOutlined } from "@ant-design/icons";
// import { useNavigate } from "react-router-dom";
// import jobsData from "../data/job_data";
// import "./job_seeker.css";
...
// const JobSeekerDashboard = () => {
//   const [currentJobs, setCurrentJobs] = useState(jobsData);
//   const [appliedJobs, setAppliedJobs] = useState([]);
//   const navigate = useNavigate();
...
//     const stored = JSON.parse(localStorage.getItem("appliedJobs")) || [];
//     setAppliedJobs(stored);
//     setCurrentJobs(jobsData.filter((job) => !stored.some((a) => a.id === job.id)));
//   }, []);

//   const handleApply = (job) => {
//     const storedJobs = JSON.parse(localStorage.getItem("appliedJobs")) || [];
//     if (storedJobs.some((j) => j.id === job.id)) {
//       message.info(`You already applied for "${job.title}".`);
//       return;
//     }

//     const updatedApplied = [...appliedJobs, job];
//     setAppliedJobs(updatedApplied);
//     setCurrentJobs(currentJobs.filter((j) => j.id !== job.id));
//     localStorage.setItem("appliedJobs", JSON.stringify(updatedApplied));
//     message.success(`You applied for "${job.title}" successfully!`);
//   };
...
//         <div className="header-left">
//           <Avatar size="large" icon={<UserOutlined />} />
//           <Title level={3} className="dashboard-title">JobSeeker Dashboard</Title>
//         </div>
...
//                     currentJobs.map((job) => (
//                       <Card key={job.id} title={job.title} className="job-card" hoverable>
...
//                         <Button type="primary" block onClick={() => handleApply(job)}>Apply Now</Button>
//                       </Card>
//                     ))
//                   ) : (
//                     <Empty description="No current openings available." />
//                   )}
//                 </div>
//               ),
//             },
...
//                   {appliedJobs.length > 0 ? (
//                     appliedJobs.map((job) => (
//                       <Card key={job.id} title={job.title} className="job-card applied" hoverable>
//                         <p><strong>Company:</strong> {job.company_name}</p>
//                         <p><strong>Location:</strong> {job.location}</p>
//                         <p><strong>Type:</strong> {job.employment_type}</p>
//                         <p><strong>Experience:</strong> {job.experience_level}</p>
